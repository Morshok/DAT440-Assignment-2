...
class Agent(object):
    ...
    def observe(self, observation, next_action, reward, done):
        self.Q[self.current_state, self.current_action] += self.alpha * (reward + self.gamma * np.argmax(self.Q[observation, next_action]) - self.Q[self.current_state, self.current_action])
